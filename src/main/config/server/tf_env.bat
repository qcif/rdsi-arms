...
set SERVER_ENVIRONMENT=${environment}
set NODE_THEME="${theme}"
...
set EXTRA_OPTS=-Dserver.url.base="%SERVER_URL%" -Dserver.environment=%SERVER_ENVIRONMENT% -Dnode.theme=%NODE_THEME% -Damq.port=%AMQ_PORT% -Damq.stomp.port=%AMQ_STOMP_PORT% -Dsmtp.host="%SMTP_HOST%" -Dadmin.email="%ADMIN_EMAIL%" -Dredbox.version="%REDBOX_VERSION%"
